...
hadron asset list -m <memory> [--mine] [--mime <type>] [--include-deleted] [--limit N] [--offset N] | get <asset-ref> [-o <path>|-] [--force] | url <asset-ref> [-m <memory>] | upload <file> -m <memory> [--mime <t>] [--name <n>] [--description <d>] | rm <asset-ref> [--yes] | restore <asset-ref> | link <asset-ref> --node <node-urn> [--name <n>] [--description <d>]
...
hadron coding review list -m <memory> [--root <loc>] [--broken] [--json] | review create <check-name> -m <memory> --trigger <cond> --description <d> [--scope <s>] [--tag <t>]... [--link <ref>[=<label>]]... [--seq N] [--content <text|-> | --content-file <path>] | review lint -m <memory> [--root <loc>] [--toolchain <t>|-] [--strict] [--suggest] [--fix [--yes]] [--json] | preflight list -m <memory> [--root <loc>] [--broken] [--json] | preflight lint -m <memory> [--root <loc>] [--strict] [--json]
...
- `hadron asset` is the file surface (spec 006 / cor:dmo:060:10). An asset is
  ...
  "restorable" rather than "permanent". `asset link <ref> --node <node-urn>`
  creates a `nodeType: reference` node under that parent carrying the asset in
  `data.asset`, the same shape `hadron_store_file` writes; the asset and the
  node need not share a memory (READ on the asset's, WRITE on the target's).
