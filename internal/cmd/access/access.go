...
// support yet — hadron-server#325). An id- or handle-shaped ref resolves via
// the uniform user(ref:) find-one first (one round trip, no paging); anything
// else — or a find-one miss — goes through users(filter: { query })
// (hadron-server#473, the searchUsers successor), which is itself
