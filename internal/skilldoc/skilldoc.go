...
// the pre-#580 procedure wrote). Only those are preamble: a body that begins
// with an HTML comment of its own keeps it, or a fresh export would parse to
// a different body than it hashed (Codex on #589, round 3).
func isProvenanceComment(t string) bool {
	if !strings.HasPrefix(t, "<!--") || !strings.HasSuffix(t, "-->") {
		return false
	}
	return strings.Contains(t, "hadron-skill") || strings.Contains(t, "Generated from") ||
		strings.Contains(t, "Generated by `hadron skill export`") || strings.Contains(t, "Edit the source node")
}
