...
1. **Resolve `$ARGUMENTS` to one node URN:**
   - If `$ARGUMENTS` is already a fully-qualified URN (`hrn:node:…`, or `org::memory::loc`), use it directly.
...
3. **Open it:** run `open "https://hadronmemory.com/app/u/<urn>"` (macOS) to launch the default browser, and also print the URL so the user can click it.
