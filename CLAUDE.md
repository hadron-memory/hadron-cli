...
- **Every target that reads `../hadron-server` states the revision it read, and refuses a revision nobody chose (#503).** `HADRON_SERVER_DIR` names a *directory* while correctness depends on a *revision* — and that sibling is shared, so it is regularly on a colleague's unmerged branch. Exporting from one produces valid SDL that compiles and passes the tests while carrying a contract that is not on `main`. `scripts/sibling-source.sh` prints `schema: ../hadron-server @ <sha> (<branch>, == origin/main)` and, **when you did not set `HADRON_SERVER_DIR` yourself**, fails unless the checkout is at `origin/main` with the files that target reads uncommitted-clean. Setting the variable is the deliberate signal — CI does (a throwaway clone), and so should you when pointing at a worktree of `origin/main` or testing against an unmerged server branch on purpose.
